        ...
        <Card className={classes.markerCard} onClick={() => setListingDialogOpen(true)}>
            <CardActionArea>
                ...
                <><Paper className={classes.avatar} >
                  ...
                  <CardMedia className={classes.media} image={props.listing.images[0].image_medium} />  </> : null}
...
function HomePageMap(props) {
   ...
   const renderMarkers = (data) => {
     return data.map(listing => <MapLocation key={listing.id} listing = {listing} lat={listing.lat} lng={listing.lng}/>)
   }
