    ...
    return (
        <>
            <Grid item xs={12} sm={12} md={12} style={{ display: "flex", alignItems: "top", marginTop: 10}}>
              <Typography variant="subtitle2"  style={{ flex: 1 }}>{props.listing.street_building}, {props.listing.district}</Typography>
              <Typography variant="body2" style={{fontWeight: 'bold'}}> {props.listing.currency==="USD" ? '$' : props.listing.currency==="RUB" ? '₽' : '֏'}{props.listing.price.toLocaleString()} {props.listing.transaction === "Rent" && `/ ${props.listing.rent_period === "Monthly" ? "Month" : "Day"}`}</Typography>
            </Grid>
            ...
            <Grid container alignItems="center" className={classes.generalInfo}>
                ...
                {props.listing.home_type === "Apartment" && <> <Typography variant="body2">Floor: {props.listing.floor}/{props.listing.number_of_floors}</Typography>
                <Divider orientation="vertical" flexItem /> </>}
                {props.listing.home_type !== "Apartment" && <> <Typography variant="body2">Floors: {props.listing.number_of_floors}</Typography>
                <Divider orientation="vertical" flexItem /> </>}
                <Typography variant="body2">Furnishing: {props.listing.furnishing}</Typography>
                <Divider orientation="vertical" flexItem />
                {props.listing.home_type !== "Apartment" && <> <Typography variant="body2">Lot size: {props.listing.lot_size}㎡</Typography>
                <Divider orientation="vertical" flexItem /> </> }
            ...
            <Grid item xs={12} sm={12} md={12} style={{ display: "flex", marginTop: 5, padding: 0, }}>
              <Typography color="textSecondary" style={{ flex: 1, fontSize: 13}}>
                ...
                { !props.myListings && <>, By: <Button size="small" color="primary" className={classes.button} onClick={handleClick}> {props.listing.owner.username} </Button>
                <Popover
                  ...
                  >
                      <Typography variant="caption" style={{fontWeight: "bold"}}>{props.listing.owner.first_name} {props.listing.owner.last_name}</Typography><br/>
                      <Typography variant="caption">Email: {props.listing.owner.email}</Typography> <br/>
                </Popover> </>}
              </Typography>
              {!props.homePage && <Typography color="textSecondary" variant="caption" >ID: {props.listing.id}</Typography>}
            </Grid>
        </>
    )
}
